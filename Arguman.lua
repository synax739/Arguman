-- // MM2 Mobil – Geliştirilmiş: Gun ESP (GunDrop) + Anlık Aim + Özel Panel + Jump Butonu
...
local ESPData = {}
local gunESPObjects = {}   -- GunDrop çizimleri burada
...
local function createESP(plr)
    ...
    if d.dist then d.dist.Size = 13 d.dist.Center = true d.dist.Outline = true end
...
local function updateGunESP()
    ...
    if not cfg.gun_esp then return end
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Name == "GunDrop" then
            ...
            if dist > cfg.esp_maxDist then continue end
...
local function aimAt(targetPlayer)
    ...
    local targetPos = hrp.Position + Vector3.new(0, 1, 0) -- gövde
...
-- ==============================================
-- PANEL (170x140, başlıktan sürüklenebilir)
-- ==============================================
...
RunService.RenderStepped:Connect(function()
    ...
    -- AIMBOT
    if cfg.aim_on and myRole == "Sheriff" and hasGun() then
        local target = getClosestMurderer()
        if target then aimAt(target) end
    end

    -- SPEED (sürekli kontrol)
...
print("✅ MM2 Panel + Gun ESP (GunDrop) + Anlık Aim + Jump Butonu hazır!")
