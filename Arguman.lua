-- MM2 - ESP + Gun ESP + Şerif Aim + Speed Hack + SILAH KAPMA (OTOMATİK TELEPORT)
...
local gunESPObjects = {}
local ESPData = {}
local grabbedGun = nil   -- Alınan silah objesi (geçici)
local grabCooldown = 0  -- Cooldown süresi (saniye)
...
-- ==============================================
-- YENİ: SILAH KAPMA (GUN GRAB)
-- ==============================================
local function grabGun()
    -- Sadece şerif rolünde ve silah yoksa çalışsın
    if getPlayerRole(LocalPlayer) ~= "Sheriff" then return end
    if hasGun() then return end  -- Zaten silah varsa yapma
    if grabCooldown > tick() then return end  -- Cooldown kontrolü
    ...
    if not closestGun then return end
    if closestDist > 50 then return end  -- 50 metre içinde değilse alma

    -- Mevcut pozisyonu kaydet
    local originalCFrame = hrp.CFrame

    -- Silahın üzerine ışınlan
    hrp.CFrame = CFrame.new(closestGun.Position + Vector3.new(0, 2, 0))
    wait(0.15)  -- Silahın alınması için bekle

    -- Silahı al (Tool olarak eklenmiş mi kontrol et)
    local success = false
    for _ = 1, 3 do
        wait(0.1)
        if hasGun() then
    ...
    if success then
        print("✅ Silah başarıyla alındı!")
    else
        print("❌ Silah alınamadı, tekrar dene.")
    end
end
...
local function createPanel()
    ...
    panel.Size = UDim2.new(0, 320, 0, 360)  -- Yükseklik arttı
    ...
    addToggle(sheriffPage, "Silah Kapma (Teleport)", cfg.gun_grab, function(v)
        cfg.gun_grab = v
    end, 43)  -- Yeni toggle
